...
function readEnv() {
  ...
  if (kind !== "frame" && kind !== "video" && kind !== "gif") {
    fail("ENV003", `KIND must be 'frame', 'video' or 'gif' (got '${kind}')`);
  }
  let params = {};
...
async function runHttpServer() {
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on("end", async () => {
      ...
      if (env.kind !== "frame" && env.kind !== "video" && env.kind !== "gif") {
      ...
      try {
        ...
        const ext = env.kind === "frame" ? "png" : env.kind === "gif" ? "gif" : "mp4";
        const mime = env.kind === "frame" ? "image/png" : env.kind === "gif" ? "image/gif" : "video/mp4";
