...
async function renderMesh(env) {
  ...
  try {
    ...
    const b64 = await page.evaluate(
      ([s, d, z]) => window.__svgToGlbB64(s, d, z),
      [env.source, depth, size],
    );
    fs.mkdirSync(env.outDir, { recursive: true });
...
async function runHttpServer() {
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on("end", async () => {
      ...
      if (!["frame", "video", "gif", "mesh"].includes(env.kind)) {
      ...
      try {
        ...
        const ext = env.kind === "frame" ? "png" : env.kind === "gif" ? "gif" : env.kind === "mesh" ? "glb" : "mp4";
        const mime = env.kind === "frame" ? "image/png" : env.kind === "gif" ? "image/gif" : env.kind === "mesh" ? "model/gltf-binary" : "video/mp4";
