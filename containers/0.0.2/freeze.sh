...
# Lucide icon catalog — `icon("<name>")` fetches /icons/<name>.svg at render
# time, so the runner needs the set baked in (generated by copy-icons.sh).
if [[ -d "$dist_dir/icons" ]]; then
  cp -R "$dist_dir/icons"            "$viewer_dir/icons"
fi
