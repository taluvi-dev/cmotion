...
const iconSvgCache = new Map<string, string>(); // lucide name → raw svg ("" = unknown)
...
function svgWeight(f: Record<string, JsonNode>): number | null {
  return f.weight?.kind === "number" ? numberOf(f.weight) : null;
}

// Recolour (`currentColor` → colour), set stroke-width (weight), force the
...
function svgTexture(svg: string, color: string | null, weight: number | null): THREE.Texture {
  const key = `${color ?? ""}|${weight ?? ""}|${svg}`;
  const cached = svgTexCache.get(key);
  if (cached) return cached;
  const tex = new THREE.Texture();
  tex.colorSpace = THREE.SRGBColorSpace;
  rasterizeInto(tex, svg, color, weight);
  svgTexCache.set(key, tex);
  return tex;
}

// icon(name, ...) rides the same renderer: fetch the bundled Lucide SVG
// (/icons/<name>.svg, generated by scripts/copy-icons.sh), then rasterise.
// Cached per (name, colour, weight) so the per-frame rebuild doesn't refetch.
function iconTexture(name: string, color: string | null, weight: number | null): THREE.Texture {
  ...
  void (async () => {
    let svg = iconSvgCache.get(name);
    if (svg === undefined) {
      try {
        const r = await fetch(`/icons/${encodeURIComponent(name)}.svg`);
        svg = r.ok ? await r.text() : "";
      } catch {
        svg = "";
      }
      iconSvgCache.set(name, svg);
    }
    if (svg) rasterizeInto(tex, svg, color, weight);
    else console.warn(`[cmotion-viewer] icon: unknown name "${name}"`);
  })();
  return tex;
}

// Shared quad for svg()/icon(): size from size:/width:/height: (default 256px
// square), animatable opacity:.
function svgQuad(texture: THREE.Texture, f: Record<string, JsonNode>): THREE.Object3D {
...
// svg(source, size?|width?/height?, color?, weight?, opacity?) — a vector
// image rasterised to a textured quad. `source` is an SVG string (positional
// or `src:`); `color:` recolours `currentColor`, `weight:` sets stroke-width.
function buildSvg(node: JsonNode): THREE.Object3D | null {
  ...
  return svgQuad(svgTexture(svg, svgColor(f.color ?? f.fill), svgWeight(f)), f);
}

// icon(name, size?, color?, weight?, opacity?) — a Lucide icon by name.
function buildIcon(node: JsonNode): THREE.Object3D | null {
  const f = namedFields(node);
  const raw = positionalArgs(node)[0] ?? f.name;
  const name = raw && raw.kind === "string" ? String(raw.raw).replace(/^"|"$/g, "") : null;
  if (!name) {
    console.warn(`[cmotion-viewer] icon: no name`);
    return null;
  }
  return svgQuad(iconTexture(name, svgColor(f.color ?? f.fill), svgWeight(f)), f);
}
