...
scene title(duration: Duration = 6s) -> Frame {
  ...
  let glyph = extrude(text.glyph("C", font: "Inter Bold"), depth: 32px)
...
- **3D primitives** (`extrude(text.glyph("C"), depth: 32px)`) — `mesh3d` is part of the [stdlib](/language/stdlib/); 3D scenes project back to a 2D `Layer` via `render3d(...)` so they `compose` alongside 2D layers.
