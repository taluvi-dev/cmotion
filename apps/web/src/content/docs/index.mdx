...
- **3D primitives** (`extrude(text.glyph("C"), depth: 80px)`) — `mesh3d` is part of the [stdlib](/language/stdlib/); 3D scenes project back to a 2D `Layer` via `render3d(...)` so they `compose` alongside 2D layers.
